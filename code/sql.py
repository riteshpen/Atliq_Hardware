...
# Define queries
queries = {
    "1": "SELECT market FROM dim_customer WHERE region = 'APAC' and customer = 'Atliq Exclusive';",
    "2": """WITH yearly_counts AS (
    ...
    "3": "SELECT segment, count(distinct product_code) as product_counts FROM dim_product GROUP BY segment ORDER BY product_counts DESC;",
    "4": """WITH yearly_counts AS (
                SELECT dp.segment,
    ...
    "6": """SELECT pre.customer_code, cust.customer, avg(pre.pre_invoice_discount_pct) as average_discount_percentage 
    ...
    "7": """SELECT MONTH(sale.date) AS sale_month, YEAR(sale.date) AS sale_year, SUM(sale.sold_quantity) AS sale_gross_sales_amount
    ...
    "8": """SELECT SUM(sold_quantity) AS total_sold_quantity,
                CASE 
    ...
    "9": """WITH cte AS (SELECT dc.channel, SUM(fsm.sold_quantity * fgp.gross_price) AS gross_sales
    ...
    "10": """CREATE TEMPORARY TABLE temp_sales
...
if st.button('Run Query'):
    ...
    st.write(f"Results for query {query_choice}:")
    st.dataframe(df)
